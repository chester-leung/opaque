...
trait OpaqueOperatorTests extends FunSuite with BeforeAndAfterAll { self =>
  ...
  //   val data = for (i <- 0 until 256) yield(i, abc(i))
  //   val df = makeDF(data, securityLevel, "id", "word")
  //   df.filter($"id".between(100, 200)).explain
  // }
  // 
  // testAgainstSpark("in") { securityLevel =>
  //   val data = for (i <- 0 until 256) yield(i, abc(i))
  //   val df = makeDF(data, securityLevel, "id", "word")
  //   df.filter($"word".isin("A", "B")).explain
  // }
